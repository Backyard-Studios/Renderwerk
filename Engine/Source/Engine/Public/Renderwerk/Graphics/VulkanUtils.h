...
#define CHECK_VKRESULT(ResultExpression, ...) \
	{ \
		VkResult UNIQUE_NAME(VulkanResult) = ResultExpression; \
		if (UNIQUE_NAME(VulkanResult) != VK_SUCCESS) [[unlikely]] \
		{ \
			RW_LOG_ERROR("{}", ToString(UNIQUE_NAME(VulkanResult))); \
			return FResult(RESULT_VULKAN_FAILED_RESULT, std::format(__VA_ARGS__).c_str()); \
		} \
	}

ENGINE_API INLINE std::string ToString(const VkResult Result)
{
	switch (Result)
	{
...
ENGINE_API INLINE std::string ToString(const VkFormat Format)
{
	switch (Format)
	{
...
ENGINE_API INLINE std::string ToString(const VkColorSpaceKHR ColorSpace)
{
	switch (ColorSpace)
	{
...
ENGINE_API INLINE std::string ToString(const VkPresentModeKHR PresentMode)
{
	switch (PresentMode)
	{
